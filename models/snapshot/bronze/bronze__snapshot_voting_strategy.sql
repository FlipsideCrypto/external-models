...
ready_prop_requests AS (
    SELECT
        'query { proposals(orderBy: "created", orderDirection: asc, first: 1000, where:{created_gte: ' || max_time_start || ',created_lt: ' || max_time_end || '}) { id space{id voting {delay quorum period type}} created start end } }' AS proposal_request_created
    FROM
        (
            SELECT
                DATE_PART(
